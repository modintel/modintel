...
`, cfg.Paranoia, cfg.BlockingParanoia, cfg.AnomalyInbound)
	...
	ruleEnginePath := filepath.Join(dir, "rule-engine.conf")
	ruleEngineContent := fmt.Sprintf("# Generated by ModIntel\nSecRuleEngine %s\n", cfg.RuleEngine)
	if err := os.WriteFile(ruleEnginePath, []byte(ruleEngineContent), 0644); err != nil {
		return err
	}
